...
\begin{abstract}
\begin{enumerate}
\item Ecological evidence can estimate a present state sharply while still failing to justify a unique claim about the future. We study \emph{finite reportability}: for a declared ecological prediction target, what can a realized finite record honestly support?
\item We represent a finite evidence problem by latent worlds, experiment records, declared future actions, observation-failure architecture, costs, and a target map. We derive the experiment-induced compatible-world quotient, the unique coarsest action-stable target-safe quotient, failure-aware guarantees for trustworthy refinement, and an adaptive false-resolution reporting contract.
\item A 16-world exact benchmark shows that full-world entropy reduction can spend information on a precisely measured but target-irrelevant latent attribute and leave the focal prediction unresolved. Equal-effort detection designs also yield different certifiable target evidence when reads share versus diversify failure modes. All headline results are verified by finite enumeration, deterministic replay, and a formal Supplement; a posterior-sample demonstration connects the finite interface to continuous Bayesian model output.
\item The method is not a replacement for occupancy models, goal-oriented experimental design, value of information, prediction sets, or state abstraction. Its contribution is to connect those adjacent ideas through one auditable reporting object: the target values still compatible with the realized evidence, the minimal latent distinctions needed to preserve them under future actions, and the observation/risk conditions required before a singleton prediction can be defended. Otherwise, the honest ecological prediction remains a set.
\end{enumerate}
\end{abstract}
