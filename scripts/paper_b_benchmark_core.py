"""Exact likelihood-based benchmark for target-safe ecological prediction."""
...
def mutual_information(belief: dict[World, float], kernel: Kernel) -> float:
    ...
    for outcome, mass in predictive_distribution(belief, kernel).items():
        if mass > 0.0:
            expected += mass * entropy(posterior(belief, kernel, outcome).values())
    return entropy(belief.values()) - expected
...
def risk_limited_report(belief: dict[World, float], limit: float) -> frozenset[str]:
...
def belief_after_screen(p: Parameters, architecture: str, detections: tuple[bool, ...]) -> dict[World, float]:
    return normalize({
        world: mass * screen_distribution(world, p, architecture).get(detections, 0.0)
        for world, mass in prior(p).items()
    })
    ...
    for world, world_mass in prior(p).items():
        ...
        for detections, screen_mass in screen_paths(world, p, architecture):
            ...
            for name, plan in plans.items():
                for report, followup_mass, followup_cost in followup_distribution(world, belief, plan, p.false_resolution_limit):
                    path_weight = weight * followup_mass
                    totals[name][classify(report, truth)] += path_weight
...
def write_outputs(report: dict[str, object]) -> None:
    ...
    slash = chr(92)
    lines = [
        f"{slash}begin{{tabular}}{{lrrrr}}",
        f"{slash}toprule",
        f"Strategy & Correct & Wrong & Ambiguous & Cost {slash}{slash}",
        f"{slash}midrule",
    ]
    for row in baseline:
        label = row["strategy"].replace("_", f"{slash}_")
        lines.append(f"{label} & {row['correct_probability']:.3f} & {row['wrong_probability']:.3f} & {row['ambiguity_probability']:.3f} & {row['expected_cost']:.2f} {slash}{slash}")
    lines.extend((f"{slash}bottomrule", f"{slash}end{{tabular}}"))
    OUT_TEX.write_text("\n".join(lines) + "\n", encoding="utf-8")
